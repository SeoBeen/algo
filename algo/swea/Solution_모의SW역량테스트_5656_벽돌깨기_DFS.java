...
public class Solution_모의SW역량테스트_5656_벽돌깨기_DFS {
	
	private static int T,N,W,H, min;
...
//	중복 순열로 구슬을 던짐
	private static void go(int cnt, int[][] map) {
		...
		for(int c = 0; c<W; c++) { // 0열부터 마지막 열까지 시도
			...
			if(r == H) {	// 구슬을 던졌지만 맞은 벽돌이 없는 경우(해당 열이 모두 빈칸)
	...
	private static void boom(int[][] map, int r, int c, int cnt) {
		...
		if(cnt == 1) return;	// 현 위치의 벽돌숫자가 1이면 주변 연쇄작용 없으므로 리턴
